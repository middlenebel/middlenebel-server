...
Config::Config(Lexical* lex, Component* parent):Component(parent){
    config = this;
    this->lex = lex;
    init();
...
string Config::cfg(string key, string defValue){
    ...
    if (useConfig.empty()){
        LOG("ERROR useConfig undefined! Using 'Default' instead.");
        useConfig = "Default";
    }
    Config* realConfig = configurations[ useConfig ];
    if (realConfig == nullptr){
        LOG("WARNING! Configuration not defined: " << useConfig << "Using global.");
        realConfig = this;
    }
...
void Config::parse(){
    ...
    LOG( "Using configuration: " << configurations[ attributes[ATT_USE_CONFIG] ]->attributes[ATT_NAME]);
    if ( IS_CONFIG( ATT_DUMP_CONFIG, CFG_TRUE)) DUMP_CONFIGURATION;
}

void Config::parseConfig(){
    //DEBUG LOG( "parseConfigs..." );
    ...
    while ( readToken() ){
        ...
        if (isBlanc()) { SEG_SHOW_BLANC(); }
            ...
            if (key.empty() && (token == ATT_USE_CONFIG)){
                LOG("ERROR in Config.nebel. 'useConfig' only can be setted in the global area.");
